...
fn get_suggestion(str: String) -> Vec<String> {
    str.replace(" or ", ", ")
        .split(", ")
        .map(|s| s.to_string())
        .collect()
...
pub fn get_new_command(command: &mut CrabCommand, system_shell: Option<&dyn Shell>) -> Vec<String> {
    ...
    .unwrap();
    let caps = re.captures(&command.stdout.unwrap()).unwrap();
    let suggestions = get_suggestion(caps.get(1).map_or("", |m| m.as_str()).to_owned());
    suggestions
        .iter()
