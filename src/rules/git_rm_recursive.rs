...
fn auxiliary_match_rule(command: &CrabCommand) -> bool {
    command.script.contains(" rm ")
        && command.output.as_ref().map_or(false, |o| {
            o.contains("fatal: not removing '") && o.contains("' recursively without -r")
        })
}
...
fn auxiliary_get_new_command(
    ...
    if let Some(index) = command_parts.iter().position(|&r| r == "rm") {
        command_parts.insert(index + 1, "-r");
    }
...
mod tests {
    ...
    #[rstest]
    #[case("git rm foo", "fatal: not removing 'foo' recursively without -r", true)]
    #[case(
        "git rm foo bar",
