...
fn auxiliary_match_rule(command: &CrabCommand) -> bool {
    if let Some(output) = &command.output {
        ...
        (output_lower.contains("did you mean this?") || output_lower.contains("did you mean one of these?")) || (command.script_parts.contains(&"install".to_owned()) && output_lower.contains("composer require"))
    } else {
        false
    }
}
...
pub fn auxiliary_get_new_command(command: &CrabCommand) -> Vec<String> {
    ...
    let broken_cmd;
    let mut new_cmd;
    if command.script_parts.contains(&"install".to_owned()) && command.output.as_ref().unwrap().to_lowercase().contains("composer require") {
        broken_cmd = "install".to_owned();
        new_cmd = "require".to_owned();
    } else {
        broken_cmd = re_broken_cmd.captures(&command.output.as_ref().unwrap()).unwrap().get(1).map_or("", |m| m.as_str()).to_owned();
        new_cmd = re_new_cmd.captures(&command.output.as_ref().unwrap()).unwrap().get(1).map_or("", |m| m.as_str()).to_owned();
        if new_cmd.is_empty() {
            new_cmd = re_new_cmd_alt.captures(&command.output.as_ref().unwrap()).unwrap().get(1).map_or("", |m| m.as_str()).to_owned();
        }
    }
